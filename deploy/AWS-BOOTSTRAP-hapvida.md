...
| G6 | EventBridge Scheduler schedule | `khal-hapvida-prod-usage-backup` — `cron(0 7 * * ? *)` tz `America/Sao_Paulo`, flexible window OFF, target = cluster A1, task-definition ARN **revision-less** `…:task-definition/khal-hapvida-prod-usage-backup`, Fargate, role D4; `awsvpcConfiguration`: `Subnets` = the two B4 **`subnet-…` IDs** (exact, no stray space), `SecurityGroups` = B10's **`sg-…` ID — the ID, not the name `khal-hapvida-prod-usage-workers`**, `AssignPublicIp=DISABLED`; `RetryPolicy.MaximumRetryAttempts` **3**. Scheduler stores whatever it is given and RunTask rejects a name or a space only at 07:00, with no task and no failure event — preflight fails on both |
